...
@endif
    ...
    <form  class="flex flex-col w-[600px] item-start justify-start" action="{{ route('novedades.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="flex items-start flex-col w-full mt-4">
                ...
                <input class="w-full block py-2 px-2 border rounded border-cyan-950"  required  type="text" id="titulo" name="titulo" />
            </div>
            <div class="flex items-start flex-col w-full mt-4">
                <label class="mb-3" for="contenido">Contenido</label>
                <textarea   required class="w-full block py-2 px-2 border resize-none rounded border-cyan-950" cols="30" rows="5" name="contenido" id="contenido"></textarea>
            </div>
            <div class="flex items-start flex-col w-full mt-4">
